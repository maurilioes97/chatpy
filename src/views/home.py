...
def render_home():
    ...
    Este e um chatbot inteligente alimentado por **Google Gemini AI**.

    **Recursos:**
    - 💬 Conversas ilimitadas
    - 📚 Historico de mensagens
    - 🚀 Respostas rapidas com IA
...
def _render_logged_home():
    """Exibe a home para usuario ja autenticado."""
    username = st.session_state.get("username", "usuario")
    st.subheader("✅ Sessao ativa")
    st.write(f"Voce esta logado como **{username}**.")

    open_col, logout_col = st.columns(2)

    if open_col.button("Abrir chat", type="primary", use_container_width=True):
    ...
    st.info("Voce pode alternar entre esta tela inicial e a tela de chat quando quiser.")

    with st.expander("Zona de perigo"):
        st.warning("Excluir a conta remove tambem todas as conversas e mensagens salvas.")
        ...
        if st.button("Excluir minha conta", use_container_width=True):
            if confirmation_name.strip() != username:
                st.error("❌ Confirmação invalida. Digite exatamente seu nome de usuario.")
            else:
...
def _render_auth_tabs():
    ...
    with tab1:
        ...
        if login_submitted:
            ...
            if not normalized_username:
                st.warning("⚠️ Digite um nome de usuario")
            elif not password:
    ...
    with tab2:
        with st.form("register_form", clear_on_submit=False):
            ...
            register_submitted = st.form_submit_button("Registrar", type="primary", use_container_width=True)
